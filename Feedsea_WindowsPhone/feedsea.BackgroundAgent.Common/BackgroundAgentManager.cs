...
namespace feedsea.BackgroundAgent.Common
{
    ...
    public class BackgroundAgentManager
    {
        ...
        {
            _feedlyUserId = (string)IsolatedStorageSettings.ApplicationSettings["GeneralSettings_UserIdSetting"];
            _feedlyToken = (string)IsolatedStorageSettings.ApplicationSettings["GeneralSettings_OAuthTokenSetting"];

            if (IsolatedStorageSettings.ApplicationSettings.Contains("LiveTileSettings_TileModeSetting"))
                _tileMode = (TileMode)IsolatedStorageSettings.ApplicationSettings["LiveTileSettings_TileModeSetting"];
            else
        ...
        public void DownloadAndUpdateTiles()
        {
            var currentTiles = ShellTile.ActiveTiles;
                    ...
                    using (var storage = IsolatedStorageFile.GetUserStoreForApplication())
                    {
                        foreach (var t in currentTiles)
                        {
                            ...
                            if (data.Front != null && data.Back != null)
                            {
                                ...
                                t.Update(BuildTiles(data, storage));
                            }
        ...
        public ShellTileData BuildTiles(LiveTileData data, IsolatedStorageFile storage)
        {
            ...
            {
                WideBackgroundImage = new Uri("isostore:" + frontWideFilepath, UriKind.RelativeOrAbsolute),
                WideBackBackgroundImage = new Uri("isostore:" + backWideFilepath, UriKind.RelativeOrAbsolute),
                BackgroundImage = new Uri("isostore:" + frontMediumFilepath, UriKind.RelativeOrAbsolute),
                BackBackgroundImage = new Uri("isostore:" + backMediumFilepath, UriKind.RelativeOrAbsolute)
            };
        }
        ...
        private async Task<LiveTileData> DownloadData(Uri source)
        {
            ...
            else
            {
                ...
                var query = src.Contains("open=") ? HttpUtility.UrlDecode(src.ToString().Split(new string[] { "open=" }, StringSplitOptions.RemoveEmptyEntries)[1]) : "";
                urlId = query;
            ...
            {
                Count = count == null ? 0 : count.Count,
                NavigationUri = source,
                Type = type,
                Title = title
            };
            ...
            if (stream.Items.Any())
            {
                ...
                {
                    Favicon = tileSub.Website,
                    SourceID = tileSub.Id,
                    Image = front.Visual == null || front.Visual.Url == "(none)" || front.Visual.Url == "none" ? null : front.Visual.Url,
                    SourceName = tileSub.Title,
                    Title = front.Title
                };
                ...
                data.Back = new TileFace()
                {
                    Favicon = tileSub.Website,
                    SourceID = tileSub.Id,
                    Image = back.Visual == null || back.Visual.Url == "(none)" || back.Visual.Url == "none" ? null : back.Visual.Url,
                    SourceName = tileSub.Title,
                    Title = back.Title
                };
            }
