...
namespace feedsea.Common.Helpers
{
   ...
   public static class GeneralHelpers
   {
      ...
      public static async System.Threading.Tasks.Task<string> TreatEmbededVideos(this string article, string youtubeClient)
      {
         ...
         switch ( youtubeClient.ToLower() )
         {
            case "default":
               ...
               break;
            case "metrotube":
               content = await doc.ChangeYoutubeToMetroTube();
               break;
            case "mytube":
               content = await doc.ChangeYoutubeToMyTube();
               break;
            case "toib":
               content = await doc.ChangeYoutubeToToib();
               break;
            case "other":
               content = await doc.ChangeYoutubeToOtherClient();
               break;
            default:
               content = doc.ChangeYoutubeToHtml5();
               break;
      ...
      private static async System.Threading.Tasks.Task<HtmlNode> ChangeYoutubeToMetroTube(this HtmlDocument doc)
      {
         return await doc.ChangeYoutubeToClient("metrotube:VideoPage?VideoID={0}", "Assets\\MetroTubePlay.png", false);
      }

      private static async System.Threading.Tasks.Task<HtmlNode> ChangeYoutubeToClient(this HtmlDocument doc, string clientUri, string clientImage, bool useFullUrl)
      {
         ...
         if ( iframes != null && iframes.Count() > 0 )
         {
            ...
            foreach ( var iframe in iframes.ToList() )
            {
               ...
               if ( mask == null )
               {
                  var res = await Windows.ApplicationModel.Package.Current.InstalledLocation.GetFileAsync(System.Text.RegularExpressions.Regex.Replace(new Uri(new Uri("ms-appx://"), clientImage).OriginalString, "^.*;component/", "").Replace('/', '\\'));
                  var bytes = (await res.OpenStreamForReadAsync()).ReadToEnd();
      ...
      private static async System.Threading.Tasks.Task<HtmlNode> ChangeYoutubeToMyTube(this HtmlDocument doc)
      {
         return await doc.ChangeYoutubeToClient("mytube:link={0}", "Assets\\MyTubePlay.png", true);
      }

      private static async System.Threading.Tasks.Task<HtmlNode> ChangeYoutubeToToib(this HtmlDocument doc)
      {
         return await doc.ChangeYoutubeToClient("toib:PlayVideo?VideoID={0}", "Assets\\ToibPlay.png", false);
      }

      private static async System.Threading.Tasks.Task<HtmlNode> ChangeYoutubeToOtherClient(this HtmlDocument doc)
      {
         return await doc.ChangeYoutubeToClient("vnd.youtube:{0}?vndapp=youtube", "Assets\\YouTubePlay.png", false);
      }
