...
#region Constructors
      public AddSourceViewModel()
      {
         ...
         selectResultCommand = new ParameterRelayCommand<SearchResultData>(SelectResult)
            {
               IsEnabled = true
            };
         saveCommand = new AsyncDelegateCommand(o => ConnectionVerifyCall(SaveSource, o, OnConnectionFail))
            {
               IsEnabled = false
            };
      }
      ...
      public void LoadData(string id)
      {
         ...
         if ( EditMode )
         {
            ...
            var result = new SearchResultData()
               {
                  Id = source.UrlID, Title = source.Name, Url = source.Link
               };
            SelectResult(result);
